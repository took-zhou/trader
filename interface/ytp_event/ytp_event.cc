...
#include "common/extern/ytp/inc/ytp_trader_struct.h"
...
void YtpEvent::OnRtnOrderHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto order_info = reinterpret_cast<YtpOrderInfoStruct *>(itp_msg.address());
  ...
  if (content != nullptr) {
    if (order_info->order_status == YtpOrderInfoStruct::kCanceled) {
#ifdef BENCH_TEST
...
void YtpEvent::OnRtnTradeHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto trade_report = reinterpret_cast<YtpOrderInfoStruct *>(itp_msg.address());
  auto &trader_ser = TraderService::GetInstance();
  auto &order_manage = trader_ser.ROLE(OrderManage);
...
void YtpEvent::OnRtnOrderInsertHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto order_insert = reinterpret_cast<YtpOrderInfoStruct *>(itp_msg.address());
  auto &trader_ser = TraderService::GetInstance();
  auto &order_manage = trader_ser.ROLE(OrderManage);
  auto &order_lookup = trader_ser.ROLE(OrderLookup);
  ...
  if (content != nullptr) {
    ...
    insert_rsp->set_index(content->index);
    insert_rsp->set_result(strategy_trader::Result::failed);
    if (order_insert->order_status == YtpOrderInfoStruct::kRejected) {
      insert_rsp->set_reason(strategy_trader::FailedReason::Fund_Shortage_Error);
    } else if (order_insert->order_status == YtpOrderInfoStruct::kNoOpened) {
...
void YtpEvent::OnRtnOrderActionHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto order_action_rsp = reinterpret_cast<YtpOrderInfoStruct *>(itp_msg.address());
  auto &trader_ser = TraderService::GetInstance();
  auto &order_manage = trader_ser.ROLE(OrderManage);
...
void YtpEvent::OnRspTradingAccountHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto account = reinterpret_cast<YtpAccountInfo *>(itp_msg.address());
  auto &trader_ser = TraderService::GetInstance();
  trader_ser.ROLE(AccountAssign).UpdateAccountStatus(account->balance, account->available, itp_msg.session_id(), itp_msg.user_id());
}

void YtpEvent::OnRspMarginRateHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto margin_info = reinterpret_cast<YtpMarginInfo *>(itp_msg.address());
...
void YtpEvent::OnRspCommissionRateHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto commission_info = reinterpret_cast<YtpCommissionInfo *>(itp_msg.address());
...
bool YtpEvent::SendEmail(const utils::OrderContent &content) {
