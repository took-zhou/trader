...
void XtpEvent::OnOrderEventHandle(utils::ItpMsg &msg) {
  ...
  if (content != nullptr) {
    if (order_info->order_status == XTP_ORDER_STATUS_CANCELED || order_info->order_status == XTP_ORDER_STATUS_REJECTED ||
      ...
      recer_sender.ROLE(Sender).ROLE(DirectSender).SendMsg(msg);
      INFO_LOG("the order be canceled, order ref: %d, volume: %ld.", order_info->order_client_id, order_info->quantity);
    }
  }
}
...
void XtpEvent::OnTradeEventHandle(utils::ItpMsg &msg) {
  ...
  if (content != nullptr) {
    ...
    auto &recer_sender = RecerSender::GetInstance();
    recer_sender.ROLE(Sender).ROLE(DirectSender).SendMsg(msg);
    INFO_LOG("the order be traded, order ref: %d, volume: %ld.", trade_report->order_client_id, trade_report->quantity);
...
void XtpEvent::OnCancelOrderErrorHandle(utils::ItpMsg &msg) {
  ...
  if (content != nullptr) {
    ...
    INFO_LOG("the order was finished, ref[%ld].", cancel_info->order_xtp_id);
