...
#include <algorithm>
#include <mutex>
#include "common/extern/gtp/inc/gtp_trader_struct.h"
...
void GtpEvent::OnRtnOrderHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto order_info = reinterpret_cast<GtpOrderInfoStruct *>(itp_msg.address());
  ...
  if (content != nullptr) {
    if (order_info->order_status == GtpOrderInfoStruct::kCanceled) {
#ifdef BENCH_TEST
...
void GtpEvent::OnRtnTradeHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto trade_report = reinterpret_cast<GtpOrderInfoStruct *>(itp_msg.address());
  auto &trader_ser = TraderService::GetInstance();
  auto &order_manage = trader_ser.ROLE(OrderManage);
...
void GtpEvent::OnRtnOrderInsertHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto order_insert = reinterpret_cast<GtpOrderInfoStruct *>(itp_msg.address());
  auto &trader_ser = TraderService::GetInstance();
  auto &order_manage = trader_ser.ROLE(OrderManage);
  auto &order_lookup = trader_ser.ROLE(OrderLookup);
  ...
  if (content != nullptr) {
    if (content->comboffset == strategy_trader::CombOffsetType::OPEN) {
      ...
      order_lookup.UpdateOpenInterest(content->instrument_id, content->index, content->user_id, -order_insert->volume, 0);
    } else if (content->comboffset == strategy_trader::CombOffsetType::CLOSE_TODAY) {
      order_lookup.UpdateOpenInterest(content->instrument_id, content->index, content->user_id, 0, -order_insert->volume);
    }
    ...
    insert_rsp->set_index(content->index);
    insert_rsp->set_result(strategy_trader::Result::failed);
    if (order_insert->order_status == GtpOrderInfoStruct::kRejected) {
      insert_rsp->set_reason(strategy_trader::FailedReason::Fund_Shortage_Error);
    } else if (order_insert->order_status == GtpOrderInfoStruct::kNoOpened) {
      insert_rsp->set_reason(strategy_trader::FailedReason::No_Opened_Order);
    }
    auto *rsp_info = insert_rsp->mutable_info();
    rsp_info->set_orderprice(order_insert->price);
    rsp_info->set_ordervolume(order_insert->volume);
...
void GtpEvent::OnRtnOrderActionHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto order_action_rsp = reinterpret_cast<GtpOrderInfoStruct *>(itp_msg.address());
  auto &trader_ser = TraderService::GetInstance();
  auto &order_manage = trader_ser.ROLE(OrderManage);
...
void GtpEvent::OnRspTradingAccountHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto account = reinterpret_cast<GtpAccountInfo *>(itp_msg.address());
  auto &trader_ser = TraderService::GetInstance();
  trader_ser.ROLE(AccountAssign).UpdateAccountStatus(account->balance, account->available, itp_msg.session_id(), itp_msg.user_id());
}

void GtpEvent::OnRspMarginRateHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto margin_info = reinterpret_cast<GtpMarginInfo *>(itp_msg.address());
...
void GtpEvent::OnRspCommissionRateHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto commission_info = reinterpret_cast<GtpCommissionInfo *>(itp_msg.address());
...
bool GtpEvent::SendEmail(const utils::OrderContent &content) {
