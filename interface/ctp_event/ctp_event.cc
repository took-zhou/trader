...
void CtpEvent::OnRtnOrderHandle(utils::ItpMsg &msg) {
  ...
  if (content != nullptr) {
    if (order->OrderStatus == THOST_FTDC_OST_Canceled) {
      ...
      rsp_info->set_orderprice(order->LimitPrice);
      rsp_info->set_ordervolume(order->VolumeTotal);
...
void CtpEvent::OnRtnTradeHandle(utils::ItpMsg &msg) {
  ...
  if (content != nullptr) {
    ...
    if (content->comboffset == strategy_trader::CombOffsetType::OPEN) {
      ...
      order_lookup.UpdateOpenInterest(content->instrument_id, content->index, content->group_id, content->user_id, -trade->Volume, 0);
    }
    ...
    auto *rsp_info = insert_rsp->mutable_info();
    rsp_info->set_orderprice(trade->Price);
    rsp_info->set_ordervolume(trade->Volume);
...
void CtpEvent::OnRspOrderActionHandle(utils::ItpMsg &msg) {
  ...
  if (content != nullptr) {
    ...
    INFO_LOG("the order was finished, ref[%s].", order_action_rsp->OrderRef);
    order_manage.DelOrder(order_action_rsp->OrderRef);
  } else {
