...
size_t OrderStates::size() { return (size_t)orderKey2StateMap.size(); }
...
std::string getOrderKey(CThostFtdcOrderField *pOrder) {
  ...
  std::string orderKey = instrumentId + connect_sym + frontId + connect_sym + sessionId + connect_sym + orderRef;
  return orderKey;
}
