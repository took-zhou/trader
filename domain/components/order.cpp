...
std::map<strategy_trader::OrderType, std::string> orderTypeMap = {{strategy_trader::OrderType::limit_LIMIT, "limit_limit"},
...
OrderContent &OrderManage::getOrderCOntentByIdentityId(const std::string &identityKey) {
  for (auto &orderMap : orderMaps) {
    ...
    if (orderContent.identityId.identity == identityKey) {
      return orderContent;
    }
  }
  static OrderContent staticOrderContent = OrderContent();
...
void OrderManage::printOrderInfo(const CThostFtdcInputOrderField &order, const std::string &orderName) { showOrderInfo(order, orderName); }
...
bool buildTotalTitles(const std::string &filePath) {
  std::string titles =
      "idectityId,frontId,sessionId,orderRef,instrumentId,"
      "price,time,direction,volumn,reason\n";
...
bool OrderSave::delOneRecordByOnRtnOrder(const OrderContent &orderContent) {
  // INFO_LOG("begin delOneRecordByOnRtnOrder, identity is[%s], orderRef is[%s],
  // state[%s]",
  //         orderContent.identityId.identity.c_str(),orderContent.orderRef.c_str(),orderContent.currentStateStr.c_str());
  //    auto& jsonCfg = utils::JsonConfig::getInstance();
  //    std::string filePath =
  //    jsonCfg.getConfig("trader","DynmicSaveOrderFilePath").get<std::string>();
  //    if(!utils::isFileExist(filePath))
  //    {
  ...
  //    {
  //        ERROR_LOG("not find identity[%s] in record
  //        file[%s]",orderContent.identityId.c_str(),filePath.c_str()); return
  //        false;
  //    }
...
bool OrderSave::saveOnRtnOrderOrderState(const OrderContent &orderContent) {
  // INFO_LOG("begin saveOnRtnOrderOrderState, identity is[%s], orderRef is[%s],
  // state[%s]",
  //         orderContent.identityId.identity.c_str(),orderContent.orderRef.c_str(),orderContent.currentStateStr.c_str());
  //    auto& jsonCfg = utils::JsonConfig::getInstance();
  //    std::string filePath =
  ...
  //        tmp["currentStateChar"] =
  //        utils::charToString(orderContent.currentStateChar);
  ...
  //        saveStream.close();
  //    }
  //    else if(utils::getFileSize(filePath) == 0 ||
  //    utils::getFileSize(filePath) == -1)
  //    {
  //        json saveContent;
  //        json tmp;
  //        tmp["identityId"] = orderContent.identityId;
  ...
  //        saveStream.close();
  //    }
  // INFO_LOG("new state save ok, identity is[%s], orderRef
  // is[%s]",orderContent.identityId.identity.c_str(),orderContent.orderRef.c_str());
  return true;
}
