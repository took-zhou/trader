...
namespace orderkey
{
    ...
    {
        string frontId = to_string(pOrder->FrontID);
        string sessionId = to_string(pOrder->SessionID);
        string orderRef = string(pOrder->OrderRef);
        string instrumentId  = string(pOrder->InstrumentID);
        string connect_sym = string("_");
        string orderKey = instrumentId+connect_sym+ \
...
void showOrderStates()
{
    ...
    static Timer showTimer = Timer();
    auto showFun = std::bind(&OrderStates::showAllOrderStates, &orderState);
    showTimer.start(5*1000,showFun);
}
